...
{{- $global := .global -}}
{{- with required "element-io.synapse.labels missing context" .context -}}
{{ include "element-io.ess-library.labels.common" (list $ .labels) }}
app.kubernetes.io/component: matrix-server
app.kubernetes.io/name: synapse-{{ .ProcessType }}
app.kubernetes.io/instance: {{ $global.Name }}-synapse-{{ .ProcessType }}
app.kubernetes.io/version: {{ .image.tag | default $.Chart.AppVersion }}
k8s.element.io/synapse-instance: {{ $global.Name }}-synapse
{{- end }}
{{- end }}
...
{{ include "element-io.ess-library.labels.common" (list $ .redis.labels) }}
app.kubernetes.io/part-of: matrix-stack
app.kubernetes.io/component: matrix-server-pubsub
app.kubernetes.io/name: synapse-redis
app.kubernetes.io/instance: {{ $global.Name }}-synapse-redis
app.kubernetes.io/version: {{ .redis.image.tag }}
{{- end }}
{{- end }}
...
{{ include "element-io.ess-library.labels.common" (list $ .haproxy.labels) }}
app.kubernetes.io/part-of: matrix-stack
app.kubernetes.io/component: matrix-server-ingress
app.kubernetes.io/name: synapse-haproxy
app.kubernetes.io/instance: {{ $global.Name }}-synapse-haproxy
app.kubernetes.io/version: {{ .haproxy.image.tag }}
{{- end }}
{{- end }}
...
{{ default (printf "%s-synapse" $global.Name ) .serviceAccount.name }}
{{- end }}
{{- end }}
...
{{ default (printf "%s-synapse-redis" $global.Name ) .redis.serviceAccount.name }}
{{- end }}
{{- end }}
...
{{ default (printf "%s-synapse-haproxy" $global.Name ) .haproxy.serviceAccount.name }}
{{- end }}
{{- end }}
...
{{- if .media.storage.existingClaim -}}
{{ .media.storage.existingClaim }}
{{- else -}}
{{ $global.Name }}-synapse-media
{{- end -}}
{{- end }}
{{- end }}
